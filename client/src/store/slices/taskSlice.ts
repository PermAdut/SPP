...
export const deleteTask = (id: number) => (dispatch: any) => {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    socket.once('tasks:delete:response', (response: SocketResponse<void>) => {
      if (response.success) {
        // Ждем обновления списка задач от сервера
        socket.on('tasks:update', updateHandler)
      } else {
        dispatch(setLoading(false))
...
// Thunk для переключения статуса выполнения
