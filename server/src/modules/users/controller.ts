...
const getAll = async (
  req: Request<object, UserResponseDto[], null>,
  res: Response,
  next: NextFunction
) => {
  try {
...
const uploadFile = async (
  ...
  try {
    ...
    const body = userDatabaseInstance.uploadPhoto(
...
const changeAdm = async (
  ...
  try {
    const body = userDatabaseInstance.changeAdminStatus(
      Number(req.params.id),
      req.body.status
    );
...
const changeDesc = async (
  ...
  try {
    const body = userDatabaseInstance.changeAdditionalData(
      Number(req.params.id),
