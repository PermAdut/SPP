...
class TaskDatabase {
  ...
  getAll(userId?: number): ITask[] {
    if (userId !== undefined) {
      // Возвращаем публичные задачи и задачи пользователя
  ...
  update(id: number, updates: Partial<Omit<ITask, 'id' | 'createdAt'>>, userId?: number): ITask[] {
    const task = this.tasks.find((t) => t.id === id)
    if (!task) throw new AppError(404, 'Task not found')
    // Проверяем доступ (только владелец может редактировать индивидуальную задачу)
    if (!task.isPublic && task.userId !== userId) {
      throw new AppError(403, 'Access denied')
    }
  ...
  delete(id: number, userId?: number): void {
    const task = this.tasks.find((t) => t.id === id)
    if (!task) throw new AppError(404, 'Task not found')
    // Проверяем доступ (только владелец может удалить индивидуальную задачу)
    if (!task.isPublic && task.userId !== userId) {
      throw new AppError(403, 'Access denied')
    }
